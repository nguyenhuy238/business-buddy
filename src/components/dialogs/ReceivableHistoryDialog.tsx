/**
 * Receivable History Dialog Component
 * Displays transaction history for a customer's receivables
 */
...
import { getCustomerReceivableTransactions } from "@/services/receivableService";
import type { Receivable, ReceivableTransaction } from "@/types";
import { formatCurrency, formatDateTime } from "@/lib/format";
import { cn } from "@/lib/utils";

interface ReceivableHistoryDialogProps {
  open: boolean;
...
const transactionTypeLabels: Record<string, { label: string; color: string }> = {
  ...
  Payment: { label: "Thu tiền", color: "bg-success/10 text-success" },
  Adjustment: { label: "Điều chỉnh", color: "bg-warning/10 text-warning" },
  Refund: { label: "Hoàn tiền", color: "bg-purple-100 text-purple-800" },
};

export function ReceivableHistoryDialog({
  open,
  onOpenChange,
  receivable,
}: ReceivableHistoryDialogProps) {
  ...
  const loadTransactions = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const data = await getCustomerReceivableTransactions(receivable.customerId);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Lịch sử giao dịch công nợ của khách hàng: <strong>{receivable.customerName}</strong>
          </DialogDescription>
        </DialogHeader>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center p-12 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                Chưa có giao dịch công nợ nào cho khách hàng này
              </p>
            </div>
          ) : (
